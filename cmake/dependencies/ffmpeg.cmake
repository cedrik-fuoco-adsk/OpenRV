...
# On macOS, externally-provided shared libraries may use @rpath install names.
# FFmpeg's configure compiles and RUNS small test binaries linked against these
# libs; without an LC_RPATH in the test binary, dyld can't locate them at runtime
# (SIGABRT / Abort trap: 6). DYLD_LIBRARY_PATH cannot be used because macOS dyld
# strips it for hardened binaries like /bin/sh before the configure script ever runs.
# For every -L<dir> flag we add a matching -Wl,-rpath,<dir> so that test binaries
# can find shared libraries at runtime. The embedded rpaths are harmless:
...
SET(_ffmpeg_extra_pkgconfig_dirs "${RV_DEPS_DAV1D_LIB_DIR}/pkgconfig")
...
# PKG_CONFIG_PATH is explicitly cleared (set to empty) in the configure environment below.
# pkg-config searches PKG_CONFIG_PATH *before* PKG_CONFIG_LIBDIR; if the inherited env has
# MSYS2 MinGW paths (e.g. from Conan's msys2 package), their openssl.pc / zlib.pc shadow
# the Conan generators .pc files and return MinGW-targeted flags incompatible with MSVC.

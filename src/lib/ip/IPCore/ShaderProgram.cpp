...
void
Program::collectSymbolNames(const Expression* expr, size_t localFunctionIndex)
{
    //
    ...
    //  passed into the program. Samplers that are associated with primary
    //  image planes require unique STs; for example a sub-sampled Y U V
    //  image has one set of STs for Y and derives the U+V STs from Y's.
    //
    //  Only source function sampler parameters are allowed to have STs
    //  generated by the renderer, all other samplers must have have some
    //  indirect mechanism or use existing ones.
    //
    ...
    const bool            morphological = F->type() == Function::MorphologicalFilter;
    ...
    for (size_t i = 0; i < exprArgs.size(); i++)
    {
        if (BoundExpression* B = dynamic_cast<BoundExpression*>(exprArgs[i]))
        {
            ...
            if (inlined) 
            {
                ...
                {
                    if (pinfo->usesST) m_outputSTSet.insert(fexpr->graphID());
                    if (pinfo->usesSize) m_outputSizeSet.insert(fexpr->graphID());

                    ostringstream name;
                    name << "_samplerExpr" << m_localFunctions.size();
                    
                    size_t newIndex = m_localFunctions.size();
                    m_localFunctions.push_back(LocalFunction(name.str(),
                                                             fexpr->graphID(),
                                                             fexpr,
                                                             pinfo->usesSampling));
    ...
    if (F->usesOutputSize()) m_needOutputSize = true;
    if (F->usesOutputST()) m_needOutputST = true;
    ...
    for (size_t q = 0; q < exprArgs.size(); q++)
    {
        ...
        if (S->isSpecial())
        {
                ...
                (inlined && S->name() == "_offset"))
            {
                ...
                m_bindingMap[B] = NameBinding(B, "vec2(0.0, 0.0)", false);
            }
            else
            {
        ...
        else
        {
            ...
            {
                //
                //  Record both the gl_TexCoord number and a mapping back
                //  to the source function expression. The mapping back is
                //  required in order to e.g. find the sampler for a givin
                //  inputImage.
                //
...
string
Program::recursiveOutputExpr(const Expression* expr, bool outputOffset)
{
    //
    ...
    for (size_t q = 0; q < exprArgs.size(); q++)
    {
        ...
        const BoundExpression* B = dynamic_cast<const BoundExpression*>(be);
...
//
void
outputUniformVariable(ostream& code, const string& type, const string& name)
{
    code << "uniform " << type << " " << name << ";" << endl;
}

void
Program::outputLocalFunction(ostream& code, const LocalFunction& l)
{
    ...
    code <<  "    "
         << (ismain ? (fragColor + " = ") : "return ")
         << recursiveOutputExpr(l.root, !ismain) 
         << ";" 
         << endl;
...
Program::compile()
{
    ...
    if (m_needOutputSize) outputUniformVariable(code, "vec2", "_windowSize"); 
    if (m_needOutputST) code << varying << " vec2 _fragCoord;" << endl;
    ...
    vertexCode << "    gl_Position = projMatrix * modelviewMatrix * vec4(in_Position, 0, 1);" << endl;

    if (m_needOutputST)
    {    
        vertexCode << "    _fragCoord.x = 0.5 * (gl_Position.x + 1.0) * _windowSize.x;" << endl; 
        vertexCode << "    _fragCoord.y = 0.5 * (gl_Position.y + 1.0) * _windowSize.y;" << endl;
    }
...
Program::validate() const
{
    if (!m_programId) 
    {
        cout << "ERROR: program validation failed because there is no program" << endl;
        return false;
    }
...
const Program::ImageAndCoordinateUnit*
findTextureAssignment (const Symbol* S,
                       const string& idhash,
                       int plane,
...
void
Program::bind(const Expression* boundExpr,
              ...
              const TwkMath::Vec2f& windowSize) const
{
    ...
    //
    //  Bind known values that come from TextureUnitAssignments or output
    //  device.
    //
            ...
            if (location >= 0)
            {
                ...
                if (Shader::debuggingType() == Shader::AllDebugInfo)
                {
                    cout << "INFO: binding variable vec2 " << name 
                         << " at " << location
                         << " :: value = "
                         << icu.uncropWidth << ", " << icu.uncropHeight 
                         << endl; 
                }
            }
            else
...
void
Program::bind2(size_t& nameCount,
               const Expression* boundExpr,
               ...
               const TwkMath::Vec2f& windowSize) const
{
    ...
    for (size_t q = 0; q < nargs; q++)
    {
        ...
        if (location == -1) continue;   // not assignable
        ...
        switch (S->type())
        {
          ...
          case Symbol::Vec2fType: glUniform2fv(location, 1, (GLfloat*)vp); break;
          case Symbol::Vec3fType: glUniform3fv(location, 1, (GLfloat*)vp); break;
          case Symbol::Vec4fType: glUniform4fv(location, 1, (GLfloat*)vp); break;

          case Symbol::BoolType:
          case Symbol::IntType:   glUniform1iv(location, 1, (GLint*)vp); break;

          case Symbol::Vec2bType:
          case Symbol::Vec2iType: glUniform2iv(location, 1, (GLint*)vp); break; 

          case Symbol::Vec3bType:
          case Symbol::Vec3iType: glUniform3iv(location, 1, (GLint*)vp); break; 

          case Symbol::Vec4bType:
          case Symbol::Vec4iType: glUniform4iv(location, 1, (GLint*)vp); break; 

          case Symbol::Matrix4fType:  glUniformMatrix4fv(location, 1, true, (GLfloat*)vp); break;
          case Symbol::Matrix3fType:  glUniformMatrix3fv(location, 1, true, (GLfloat*)vp); break;
          case Symbol::Matrix2fType:  glUniformMatrix2fv(location, 1, true, (GLfloat*)vp); break;

          case Symbol::FloatArrayType: glUniform1fv(location, S->width(), (GLfloat*)vp); break;
          case Symbol::BoolArrayType: 
          case Symbol::IntArrayType: glUniform1iv(location, S->width(), (GLint*)vp); break;
          ...
          case Symbol::Sampler3DType:
              ...
              if (const BoundSampler* bsampler = dynamic_cast<const BoundSampler*>(b))
              {
                  ...
                  if (const ImageAndCoordinateUnit* icu = 
                      findTextureAssignment(S, idhash, plane, assignments))
                  {
...
const Program*
ProgramCache::select(const Expression* A)
{
    //
    ...
    else
    {
        ...
        if (!p) TWK_THROW_EXC_STREAM("ERROR: failed to select the GL program");
        return p;
    }
}

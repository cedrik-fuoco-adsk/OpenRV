...
void 
Function::useShadingLanguageVersion(const char* glVersion)
{
    ...
    {
        cout << "ERROR: Shader::Function::useShadingLanguageVersion failed" 
             << ", glVersion = \"" << glVersion << "\""
             << endl;
...
Function::Function(const string& name, 
                   ...
                   const SymbolVector& globals,
                   size_t numFetchesApprox,
                   const string& doc)
    : m_name(name), 
      m_type(type),
      m_parameters(params),
      ...
      m_inline(type == Filter || type == MorphologicalFilter)
{
    ...
    if (type == UndecidedType) inferType();
    replaceTextureCalls();
    initResourceUsage();
}
...
Function::~Function() 
{
    ...
    for (size_t i = 0; i < m_parameters.size(); i++) delete m_parameters[i];
    for (size_t i = 0; i < m_globals.size(); i++) delete m_globals[i];
}
...
Function::inferType()
{
    ...
    else
    {
        ...
        {
            if (m_inImageParameters[i].usesSampling) m_type = Filter;
        }
...
Function::initResourceUsage()
{
    ...
    for (size_t i = 0; i < m_parameters.size(); i++)
    {
        ...
        if (s->isSampler()) m_resourceUsage.buffers++;
        if (s->isCoordinate()) m_resourceUsage.coords++;
    }
}
...
{
    //
    //  NOTE: m_hash should be the combined hash of source code from *all*
    //  languages. I.e. if this function has both a GLSL and OpenCL
    //  implementation than the hash should reflect the combined source
    //  code. This can be as simple as concatenating the source text
    //  together or using boost::hash_combine() (or TR1 if it exists) to do
    //  it.
    //
...
bool
Function::isCompiled() const
{
    return m_state && m_state->shader;
}

void 
Function::releaseCompiledState() const
{
    if (m_state)
    {
...
void
outputAnnotatedCode(ostream& o, const string& code)
{
    o << endl;
...
{
    ParsedParameter() 
        : q_in(false), q_out(false), q_const(false), special(false), requiresInline(false) {}
...
void
Function::parseDeclaration(bool createSymbols)
{
    ...
    if (regex_search(m_sourceCode, m, re))
    {
        ...
        for (size_t i = 0; i < sourceParameters.size(); i++) 
        {
            ...
            {
                if      (parts[q] == "in")    { pp.q_in = true; pp.q_out = false; }
                else if (parts[q] == "out")   { pp.q_out = true; pp.q_in = false; }
                else if (parts[q] == "inout") { pp.q_in = true; pp.q_out = true; }
                else if (parts[q] == "const") pp.q_const = true;
                else if (parts[q] == "") { parts.erase(parts.begin() + q); q--; }
            }

            if (parts.size() == 0)
            {
                TWK_THROW_EXC_STREAM("empty parameter declaration for parameter "
                                     << (i+1));
            }
            ...
            if (ptype == "inputImage")
            {
                ...
                smatch sm;
                regex usesSTRE("\\b" + name + "\\s*\\.\\s*([stpq]{1,4}|[xyzw]{1,4}|[rgba]{1,4})\\b");
                regex usesSamplingRE("\\b" + name + "\\s*\\(\\s*[^\\)]");
                regex usesLookupRE("\\b" + name + "\\s*\\(\\s*\\)");
                regex usesSizeRE("\\b" + name + "\\s*\\.\\s*size\\s*\\(\\s*\\)");
                ...
                if (pp.usesSampling) usesSampling = true;
            }

            if (pp.requiresInline) m_inline = true;
        ...
        for (size_t i = 0; i < parsedParameters.size(); i++)
        {
            ...
            //
            //  Automatically determine the parameter symbol types if needed
            //
            ...
            if (p.type == "inputImage")
            {
                if (p.q_in != true || p.q_const != true)
                {
                    ...
                    TWK_THROW_EXC_STREAM("inputImage argument \'"
                                         << p.name 
                ...
                m_inImageParameters.push_back(ImageParameterInfo(p.name,
                                                                 p.usesST,
                                                                 p.usesSize,
                                                                 p.usesSampling,
                                                                 p.usesLookup));
            ...
            else if (p.type == "outputImage")
            {
                if (p.q_in != true || p.q_const != true)
                {
                    ...
                    TWK_THROW_EXC_STREAM("outputImage argument \'"
                                         << p.name 
                                         << "\' must have qualifier \'const in\'");
                }                
                ...
                //
                //  look for occurence of foo.size or swizzle and replace
            ...
            if (createSymbols)
            {
                ...
                for (const SymbolTypeAssociation* a = symbolTypeAssociations;
                     a->glslName;
                     a++)
                {
                    if (p.type == a->glslName)
                    {
                        ...
                        if (p.q_out && p.q_in) qualifier = Symbol::ParameterInOut;
                        else if (p.q_in && p.q_const) qualifier = Symbol::ParameterConstIn;
                        else if (p.q_in && !p.q_const) qualifier = Symbol::ParameterIn;
                        else if (p.q_out) qualifier = Symbol::ParameterOut;

                        if (p.special) qualifier |= Symbol::Special;

                        Symbol* s = new Symbol((Symbol::Qualifier)qualifier, p.name, stype);
                        m_parameters.push_back(s);
                        break; // in case of typo in symbolTypeAssociations
                    }
                }
            }
        }

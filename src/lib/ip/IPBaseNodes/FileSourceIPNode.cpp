...
namespace IPCore
{
    ...
    namespace
    {
        ...
        TilingInfo getTilingInfo(const int fullFBWidth, const int fullFBHeight)
        {
            ...
            if (!SOURCE_TILING)
            {
                // When source-tiling is OFF, scale down the source texture to
                // the maximum texture size (lengthLimit) if required.
    ...
    public:
        ...
        bool hasVideo() const { return shared->hasVideo; }

        bool hasAudio() const { return shared->hasAudio; }
        ...
        if (m_workItemID)
        {
            addDispatchJob(Application::instance()->dispatchToMainThread(
                 ...
                 proxySharedMedia](Application::DispatchID dispatchID)
                {
                    ...
                    // event must be send the first one. After we must send
                    // MediaChange event to notify a FPS change before send
                    // ImageStructureChange and rangeChange. This new fps is
                    // used by layout trigges by the RangeChange event
        ...
        if (context.stereo && context.eye < m_mediaVector.size())
        {
            ...
            if (media->hasVideo())
            {
                ...
                case ChannelComponent:
                    if (!media->matchesChannel(c))
                        break;
                case LayerComponent:
                    if (!media->matchesLayer(c))
                        break;
                case ViewComponent:
                    if (!media->matchesView(c))
                        break;
                    return media;
                }
            }
        ...
        for (size_t i = 0; i < m_mediaVector.size(); i++)
        {
            ...
            if (media->hasVideo())
            {
                ...
                if (c.type == LayerComponent)
                {
                    if (c.name[0].empty())
                    {
                        // This handles the case where the view has been omitted
                        // in the session mgr and therefore is "". This happens
                        // when there is just one view for all parts/layers. So
                        // we need to determine what the defaultView is to use
                        // as the view string when attempting to match the
                        // media. Note that if we are this point in the code,
                        // all prior media match attempts have failed.
                        //
                ...
                {
                    // This handles the case where the view has been omitted in
                    // the session mgr and therefore is "". This happens when
                    // there is just one view for all parts/layers. So we need
                    // to determine what the defaultView is to use as the view
                    // string when attempting to match the media. Note that if
                    // we are this point in the code, all prior media match
    ...
    {
        //
        //  Assemble request frame: rangeOffset shifts the frame range,
        //  rangeStart forces an absolute start frame.
        //
    ...
    namespace
    {
        ...
        //
        //  A convenience routine to make each frame take longer to read, to
        //  test caching, etc.
        //
    ...
    {
        // Configure a FrameBuffer and an IPImage so they use the right alpha
        // and {un}premultiplied settings.
    ...
    IPImage* FileSourceIPNode::evaluate(const Context& context)
    {
        ...
        if (failed || empty)
        {
            ...
            catch (...)
            {
                ...
                //
                //  Otherwise we can have the situation where a frame is
                //  missing, so we try for a nearby frame, but that is corrupt
                //  or incomplete so the read fails, and we end up here.  The
                //  error image/fb should have the id of the nearby frame.
                //
                try
                {
                ...
                catch (...)
                {
                    ...
                    cerr << "ERROR: FileSource: identifiersAtFrame error: "
                            "unable to get "
            ...
            if (ids.size() == 0)
            {
                if (media->hasVideo())
                {
                    //  This should never happen, but if it does still make sure
                    //  that fbs array is not empty.
                    cerr << "ERROR: FileSource eval: unable to get any "
                            "identifiers from "
                            "movie."
                         << endl;
                }
            ...
            && !context.cacheNode)
        {
            ...
            //  throw these fbs away, but we also don't want to lock unless we
            //  have to.
            //
        ...
        if (lframe > mov->info().end)
            lframe = mov->info().end;
        if (lframe < mov->info().start)
            lframe = mov->info().start;
        ...
        // frame 2 : 2x2
        // etc.
        // Note : this debug mode is only enabled when there should not be
        // tiling.
        ...
        // 'root' can either be the full IPImage (when tiling is off) or a
        // collage of tiled IPImages (when tiling is on).
        ...
        else
        {
            ...
            //  Conceptual structure of a 2x2 tiling layout with a 3-planes
            //  buffer
            //
            //       B _____________
            //     G __|_____|____ |
            ...
            //
            //  * Only the first plane (R in this example) is put in the cache
            //  and
            ...
            for (int y = 0; y < tilingInfo.numTiles; y++)
            {
                for (int x = 0; x < tilingInfo.numTiles; x++)
                {
                    ...
                    // This will copy all attributes and planes (without
                    // allocating new pixel buffers).
                    tileFB->shallowCopy(fullFB);

                    // We use the first tile FB as the master FB and the other
                    // tile FB are just proxy buffers, ie.: they refer to a
                    // section of the master buffer.
                    ...
                    else
                    {
                        ...
                        // This can be performed on the first plane since only
                    ...
                    while (aTilePlaneFB && aFullPlaneFB)
                    {
                        ...
                        if (isMasterBuffer)
                        {
                            // Flag the master buffer (first tile) so it knows
                            // it is responsible for removing the other (proxy
                            // buffer) tiles from the cache when it gets
                            // evicted.
    ...
    IPImageID* FileSourceIPNode::evaluateIdentifier(const Context& context)
    {
        ...
        //
        //  Not an error for an audio-only source to have no IDs.  XXX should
        //  come back to this.
        //
        if (!media->hasVideo())
            m.push_back("no-source-image");

        if (m.empty())
            TWK_THROW_EXC_STREAM("FileSource: movie identifiers eval failed.");

        //
        //  Optionally add in the source name to all ids. See comment in
        //  evaluate() regarding this. Basically, this forces rendered images
        //  from different sources to be unique.
        //
    ...
    void FileSourceIPNode::audioConfigure(const AudioConfiguration& config)
    {
        ...
        for (size_t i = 0; i < m_mediaVector.size(); i++)
        {
            ...
            if (!mov->canConvertAudioRate())
            {
                ...
                // particular process pass, the blocksize has to be increased at
                // large down sample rates. Otherwise the blocksize is too small
                // for the resampler to complete its process at a single pass
                // and may return zero samples processed to give it time to
                // build up more samples internally.
                //
    ...
    size_t FileSourceIPNode::audioFillBuffer(const AudioContext& context)
    {
        ...
        //
        //  There are certain cases where the m_adevRate/m_adevLayout does not
        //  equal the context buffer rate/layout. This happens when the current
        //  media happens to have the same audio config as the last media that
        //  was played.
        //
        //  See audioConfigure in IPGraph.cpp; i.e. the notify flag will be
        //  false and so audioConfigure of FileSourceIPNode is never called and
        //  the current media will have its m_adevRate still at the default
        //  value.
        //
        //  So in some cases, this default m_adevRate/m_adevLayout might not
        //  equal the context buffer rate/layout. As such we call audioConfigure
        //  explicitly to ensure the audio config rate and layout matches the
        //  context buffer rate and layout always... (which is the audio config
        ...
        //
        //  Just an FYI; this code is only ever called when the graph viewnode
        //  is this source and the session/playback fps does not match the
        //  source fps. In all other graph topologies with higher level nodes
        //  using FileSource's as inputs, the context.fps matches the fps().
        //
    ...
    FileSourceIPNode::audioFillBufferInternal(const AudioContext& context)
    {
        ...
        for (size_t i = 0; i < m_mediaVector.size(); i++)
        {
            ...
            if (!mov->hasAudio())
                continue;
            if (mov->hasVideo() && m_noMovieAudio->front())
                continue;
        ...
        //
        //  If we don't have any audio sources or we have an invalid buffer stop
        //  now
        //
             ...
             am != audioMedia.end(); am++)
        {
            ...
            if (nread == 0)
                continue; // don't mix corrupted reads
    ...
    void FileSourceIPNode::prepareForWrite()
    {
        ...
        //
        //   Note: don't get any security bookmarks (permissions) we need to
        //   read this file when sandboxed, since they should have been added to
        //   Source by higher-level code.
        //
    ...
    float FileSourceIPNode::fps() const
    {
        ...
        for (size_t i = 0; i < m_mediaVector.size(); i++)
        {
            ...
            //  fields, which can be 0 or a very small number.  We assume here
            //  that any fps < 0.5 is a mistake.
            //
        ...
        //
        //  If we get to this point, we have no clue from the media about the
        //  fps, and the user did not specify it.  So we're going to use the
        //  defaultFPS. This is a _preference_ so different users loading the
        //  same sequence can get different FPSs, furthermore rvio does not read
        //  prefs, so it always assumes fps=24 at this point.  If we save this
        //  session and reload (possibly in rvio, or with different prefs) we
        //  want to get the same fps.  So if there was at least one piece of
        //  media, we retain the assumed fps in the property.
        //

        if (!m_mediaVector.empty())
            m_fps->front() = defaultFPS;
    ...
    void FileSourceIPNode::updateHasAudioStatus()
    {
        ...
        if (graph())
            graph()->updateHasAudioStatus(this);
    }
    ...
    void FileSourceIPNode::reloadMediaFromFiles()
    {
        ...
        // Note that if any of the media file associated with this
        // FileSourceIPNode is unreachable, then setMediaActive(false) will be
        // called. For reference : FileSourceIPNode::openMovieTask()
        //
                                    ...
                                    const SharedMediaPointer& proxySharedMedia)
    {
        ...
        if (!mov || failed)
        {
            ...
            if (!m_workItemID)
            {
                // We are not using async loading: we can safely execute the
    ...
    void FileSourceIPNode::setLoadingID(IPGraph::WorkItemID newLoadingID)
    {
        ...
        if (newLoadingID == prevLoadingID)
            return;

        if (newLoadingID)
            graph()->mediaLoadingBegin(newLoadingID);

        if (prevLoadingID)
            graph()->mediaLoadingEnd(prevLoadingID);
    }

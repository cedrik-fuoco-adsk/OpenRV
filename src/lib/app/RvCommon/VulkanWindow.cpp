...
namespace Rv
{
    ...
    namespace
    {
        ...
        // everywhere else.
        //
        // NVIDIA needs OPTIMAL. Its Linux 550+ drivers return blank pixels to
        // OpenGL for LINEAR shared images >= ~2 MiB (forum thread #349436), and
    ...
    const VulkanWindow::SharedImageInfo* VulkanWindow::getSharedImageInfo(int w, int h)
    {
        ...
        if (ImageRenderer::debugGpu())
        {
            cout << "INFO: VulkanWindow: getSharedImageInfo: (re)allocating shared image slot " << slot << " capacity " << capW << "x"
                 << capH << " for request " << w << "x" << h << " tiling=" << (optimalTiling ? "OPTIMAL" : "LINEAR") << endl;
        }

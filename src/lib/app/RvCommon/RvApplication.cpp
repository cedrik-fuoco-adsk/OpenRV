...
namespace
{
    ...
    {
        constexpr array silenced = {// In Qt6, when KVM or YubiKey or other HID device is plugged in or
    ...
    void myMsgHandlerQT(QtMsgType t, const QMessageLogContext& context, const QString& qmsg)
    {
        //
        ...
        static bool reportQWebEngineJavaScripErrors = getenv("RV_REPORT_QWEBENGINE_JAVA_SCRIPT_ERRORS") != nullptr;
        if (context.category && (strncmp(context.category, "js", 2) == 0) && !reportQWebEngineJavaScripErrors)
        {
            return;
        }
...
namespace Rv
{
    ...
    QList<QNetworkProxy> RvProxyFactory::queryProxy(const QNetworkProxyQuery& query)
    {
...
// RV lazy build third party optional customization
    ...
    RvNetworkDialog* RvApplication::networkWindow()
    {
        ...
        if (doc)
        {
            ...
            m_networkDialog->move(p.x() + s.width() / 2 - ws.width() / 2, p.y() + s.height() / 2 - ws.height() / 2);
        }
    ...
    RvDocument* RvApplication::rebuildSessionFromFiles(Rv::RvSession* s, const StringVector& files)
    {
        //
    ...
    VideoModule* RvApplication::primaryVideoModule() const { return m_desktopModule; }
    ...
    RvApplication::DispatchID RvApplication::dispatchToMainThread(DispatchCallback callback)
    {
    ...
    void RvApplication::undispatchToMainThread(DispatchID dispatchID, double maxDuration)
    {
        {
            std::lock_guard<DispatchMutex> guard(m_dispatchMutex);
    ...
    void RvApplication::heartbeat()
    {
        ...
        if (false)
            if (secs > 1.0 / 30.0)
            {
                cout << "WARNING: last timer fired " << secs << " seconds ago (" << (1.0 / secs) << " fps)" << endl;
            }
    ...
    void RvApplication::sessionFromUrl(string url)
    {
        ...
        if (arg_parse(newArgc, newArgv, "", "", RV_ARG_EXAMPLES, "", "", RV_ARG_SEQUENCE_HELP, "", "", RV_ARG_SOURCE_OPTIONS(opts), "", "",
                      "", ARG_SUBR(parseInFiles), "Input sequence patterns, images, movies, or directories ", RV_ARG_PARSE_OPTIONS(opts),
                      NULL)
            < 0)
        {
    ...
    void RvApplication::parseURL(const char* s, vector<char*>& av)
    {
        ...
        for (int i = 0; i < avNew.size(); ++i)
        {
            ...
            if (s == "-eval" || s == "-pyeval" && i < avNew.size() - 1)
            {
                ...
                if (s2.find(ok1) != string::npos || s2.find(ok2) != string::npos || s2.find(ok3) != string::npos
    ...
    void RvApplication::putUrlOnClipboard(string url, string title, bool doEncode)
    {
    ...
    std::string RvApplication::queryDriverAttribute(const std::string& var) { return ""; }

    void RvApplication::setDriverAttribute(const std::string& var, const std::string& val) {}
            ...
            if (opts.vsync && !rvDoc->vsyncDisabled())
            {
                ...
                try
                {
                    if (!d->isOpen())
                    {
                        ...
                        if (dd
                            && dd->qtScreen() == QApplication::screens().indexOf(QApplication::screenAt(rvDoc->mapToGlobal(QPoint(0, 0)))))
                        {
                            TWK_THROW_EXC_STREAM("Cannot open presentation device for the same "
                    ...
                    catch (std::exception& exc)
                    {
                        ...
                        QString baseText = QString("The presentation device (%1/%2) is busy "
                                                   "or cannot be opened.")
                                               .arg(d->module()->name().c_str())
                                               .arg(d->name().c_str());
                        QString detailedText = QString(UI_APPLICATION_NAME " failed to open or bind the presentation "
                                                                           "device (%1/%2)."
                ...
                catch (std::exception& exc)
                {
                    ...
                    box.setWindowTitle(tr(UI_APPLICATION_NAME ": Presentation Mode"));
                    QString baseText =
                        QString("The presentation device (%1/%2) failed to open.").arg(d->module()->name().c_str()).arg(d->name().c_str());
                    box.setText(baseText + "\n\n" + exc.what());
                    box.setWindowModality(Qt::WindowModal);
                    QPushButton* b0 = box.addButton(tr("Ok"), QMessageBox::AcceptRole);
                    box.setIconPixmap(QPixmap(qApp->applicationDirPath() + QString(RV_ICON_PATH_SUFFIX)).scaledToHeight(64));
    ...
    void RvApplication::rebuildDesktopVideoDevices(TwkGLF::GLVideoDevice* shareDevice)
    {
        if (!m_desktopModule)
            return;

        //
        //  Capture presentation state before any teardown. The selected screen
        //  is captured implicitly: it is Options::presentDevice (a stable device
        //  name), re-resolved after the rebuild via findPresentationDevice, so we
        //  restore by name rather than by a pointer that the rebuild may destroy.
        //
        ...
        //
        //  Rebuild the per-screen devices to the current backend. A no-op
        //  (returns false, leaves m_devices in place) when the backend has not
        //  changed; the share-device rebind below still runs so a main-view swap
        //  that keeps the same backend is honored.
        //
        const bool rebuilt = m_desktopModule->rebuildDevices(shareDevice);

        //
        //  Re-bind the controller's current main-view device as the share device
        //  on every (possibly newly created) desktop device. This runs after the
        //  rebuild so it never touches an about-to-be-destroyed device (task 4.4).
        //
        ...
        //
        //  Refresh the session graph's per-physical-device display-group registry
        //  so it references the newly created device pointers. rebuildDevices()
        //  deleted the old per-screen devices and createDesktopVideoDevices() made
        //  new ones, but the graph's DisplayGroupIPNodes (built once at startup by
        //  setPhysicalDevices) still hold the destroyed pointers. Without this,
        ...
        //  display stays black. This mirrors the startup sequence and must run on
        //  every real rebuild even when presentation is currently off, because the
        //  device pointers may change while presentation is disabled and only be
        //  bound as the output later (the reported 10 -> 8 -> 10 -> enable repro).
        //
        if (rebuilt && session)
        {
        ...
        //  (swapGLViewToVulkan / fallbackVulkanToGLView / rebuildGLView) reset the
        //  session output device to 0 while rebinding the control device, which
        //  is the root of the black second display. If presentation mode is on,
        //  re-open the presentation output on the selected screen with the new
        //  backend and bind it as the session output; a backend change also
        //  destroyed the old device, so this replaces any stale reference.
        //
    ...
    TwkApp::VideoDevice* RvApplication::findPresentationDevice(const std::string& dpath) const
    {

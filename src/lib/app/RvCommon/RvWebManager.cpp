...
QNetworkReply*
LocalNetworkAccessManager::createRequest(
	    QNetworkAccessManager::Operation operation,
	    const QNetworkRequest& request,
	    QIODevice* device)
{
    //  cerr << "createRequest url '" << request.url().toString().toStdString() << "'" << endl;
...
{
    //  cerr << "*********************************** NetworkReplyTimeoutDeleted, reply " << static_cast<QNetworkReply*>(parent()) << endl;
}

void
NetworkReplyTimeout::timeout()
{
    bool authDebug = (getenv("RV_SHOTGUN_AUTH_DEBUG") != 0);
    if (authDebug) cerr << "INFO: network reply timeout" << endl;
    ...
    : QObject(parent)
{
    ...
    for (int i = 0; i < entries.size(); ++i) httpsDir.remove(entries[i]);
    ...
    connect(m_netManager, SIGNAL(authenticationRequired(QNetworkReply*,QAuthenticator*)),
            this, SLOT(replyAuthentication(QNetworkReply*,QAuthenticator*)));
...
void
RvWebManager::httpGet(const QString& url,
                      ...
                      bool  urlIsEncoded)
{
    ...
    connect(m_netManager, SIGNAL(finished(QNetworkReply*)),
            this, SLOT(replyDone(QNetworkReply*)));
...
void
RvWebManager::httpPost(const QString& url,
                       const HeaderList& headers,
                       ...
                       QString tag)
{
    ...
    bool authDebug = (getenv("RV_SHOTGUN_AUTH_DEBUG") != 0);
    if (authDebug) cerr << "INFO: RvWebManager POST to " << UTF8::qconvert(url) << endl;
    /*
    //
...
void
RvWebManager::httpPut( const QString& url,
                       const HeaderList& headers,
                       const QByteArray& putData,
                       Session* session,
                       const QString& replyEvent,
                       const QString& authenticationEvent,
                       const QString& progressEvent,
                       ...
                       QString tag)
{
    ...
    bool authDebug = (getenv("RV_SHOTGUN_AUTH_DEBUG") != 0);
    if (authDebug) cerr << "INFO: RvWebManager PUT to " << UTF8::qconvert(url) << endl;
    /*
    //
    //  Uncommenting this will reveal passwords, so not for release!
    //
...
void
RvWebManager::replyDone(QNetworkReply* reply)
{
    if (m_replyMap.count(reply) != 1) return;
    ...
    {
        cerr << "INFO: RvWebManager reply from " << reply->request().url().toString().toStdString() << endl;
    }
    */
    ...
    if (d.session)
    {
            ...
            || !replyErrorString.isEmpty() )
        {
            ...
            if (!replyErrorString.isEmpty()) utf8 = replyErrorString.toUtf8();

            (void) d.session->userRawDataEvent(d.replyEvent.toUtf8().constData(),
                                        ctype.toUtf8().constData(),
                                        array.constData(),
                                        array.size(),
                                        utf8.constData());
        }
...
void
RvWebManager::replyRead()
{
    //  cerr << "INFO: RvWebManager: readReady signal" << endl;
}

void
RvWebManager::replyError(QNetworkReply::NetworkError error)
{
    if (reportErrors) cerr << "WARNING: reply error #" << error << endl;
}

void
RvWebManager::replySSLErrors(QList<QSslError> errList)
{
    if (reportErrors)
    {
...
void
RvWebManager::managerSSLErrors(QNetworkReply* reply, QList<QSslError> errList)
{
    bool ignore = true;

    //
    //  Ignore ssl errors either if the user asked us to, or if the request was
    //  internally generated, so the user had no chance to specify the
    //  behavior.
    //
	...
	{
	    cerr << "WARNING: "
                 << ((ignore) ? "ignoring" : "not ignoring")
                 << " SSL error: "
                 << UTF8::qconvert(err.errorString())
                 << endl;
	}
    }
}
...
void
RvWebManager::replyAuthentication(QNetworkReply*,QAuthenticator*)
{

  ...
  public:
    ...
    void run()
    {
        while (!m_done)
        {
            ...
            if (m_device && m_doSync && !m_done) m_device->syncBuffers();
    ...
    void setDevice(const VideoDevice* d)
    {
        m_device = d;
    }
...
// TODO: DARWIN
        ...
        if (f.redBufferSize() != m_red && m_red != 0)
        {
            ...
            // box.setDetailedText("You can change the default display color depth and target "
            //                     "from the preferences under Rendering->Display Output Format.\n"
            //                     "Choosing \"OpenGL Default Format\" will tell RV to ask for the "
            //                     "default prefered format for this display.");
            // box.setWindowModality(Qt::WindowModal);
            // QPushButton* b1 = box.addButton(tr("Continue"), QMessageBox::AcceptRole);
...
{
    if (m_doc) m_doc->viewSizeChanged(w, h);
    #ifdef PLATFORM_WINDOWS
...
GLView::paintGL()
{
    ...
    else
    {
        if (session->outputVideoDevice() != videoDevice())
        {
            ...
            reinterpret_cast<SyncBufferThreadData*>(m_syncThreadData)->notify();
...
GLView::event(QEvent* event)
{
    ...
    if (event->type() == QEvent::WindowActivate) m_activationTimer.start();
    ...
    if (m_activationTimer.isRunning()) 
    {
        ...
        if (event->type() == QEvent::MouseMove) m_activationTimer.stop();
    }

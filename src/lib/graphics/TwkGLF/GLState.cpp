...
GLState::createGLTexture(size_t s)
{
    ...
    {
        // this really shouldn't happen. because deleteGLTextures should have taken care of it
...
void
GLState::deleteGLTexture(const GLuint& id)
{
    std::map<size_t, size_t>::iterator it = m_activeTextures.find(id);
    if (it != m_activeTextures.end())
    {
...
const GLProgram*
GLState::activeGLProgram() const
{
    return m_activeProgram;
}

// note that the [] operator will insert an element with key p if none exists
GLPipeline* 
GLState::pipeline(const GLuint p)
{
    return &(m_state[p]);
}

...
//----------------------------------------------------------------------
// 
// A GLPipeline object corresponds to a glProgram (containing the glProgram as a member)
// its main purpose is to provides access to (sets/gets) a glProgram's uniforms
// NOTE unlike gl2, we do not maintain a stack of matrices and states for each glProgram
// before each rendering occurs, the client needs to decide the proper matrices/states to set
//
...
class GLPipeline
{
  public:
    ...
    // constructor
    //
    GLPipeline(): m_program(NULL), m_projectionIsSet(false), m_modelViewIsSet(false), m_viewportIsSet(false) {}
    GLPipeline(const GLProgram* p): m_program(p), m_projectionIsSet(false), m_modelViewIsSet(false), m_viewportIsSet(false) 
    {
        m_viewport[0] = m_viewport[1] = m_viewport[2] = m_viewport[3] = 0;
        // m_modelView and m_projection defaults to identity, see TwkMath::Mat44f
    }
    ...
    // set GL uniforms
    //
    void setUniformFloat(const char* name, size_t no, GLfloat* data);  // send GL the uniform values to the current active glProgram
  ...
  private:
    ...
    bool                m_projectionIsSet; // after the first call to glUniform("projmat") this will be true
    bool                m_modelViewIsSet;
    bool                m_viewportIsSet;
    // the following 3 are maintained only for GL 2 compatibility, see UseCurrentProjectionGL2()

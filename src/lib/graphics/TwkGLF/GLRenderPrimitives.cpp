...
RenderPrimitives::RenderPrimitives(const GLProgram* p,
                                   PrimitiveData& data,
...
void 
RenderPrimitives::init(GLVBO::GLVBOVector& vbos)
{
...
void 
RenderPrimitives::pushDataToBuffer()
{
    m_dataVBO->setupData(m_primitiveData.m_dataBuffer,
                         GL_ARRAY_BUFFER,
                         m_primitiveData.m_size);
    
    if (m_hasIndices) 
    {
        m_indexVBO->setupData(m_primitiveData.m_indexBuffer,
...
void 
RenderPrimitives::bindBuffer() const
{
...
void 
RenderPrimitives::unbindBuffer() const
{
...
RenderPrimitives::render() const 
{
    ...
    for (size_t i = 0 ; i < m_attributeList.size(); ++i)
    {
        ...
        if(loc >= 0)
        {
          glVertexAttribPointer(loc,
                                m_attributeList[i].m_dimension,
                                m_attributeList[i].m_dataType,
                                GL_FALSE,
        ...
        {
          std::cerr << "ERROR: Invalid attribute location in RenderPrimitives::render." << std::endl;
        }
    }
    TWK_GLDEBUG;
    ...
    if (m_hasIndices)
    {
        glDrawElements(m_primitiveData.m_primitiveType,
                       m_primitiveData.m_primitiveNo * m_verticesPerPrimitive,
                       m_indexDataType,
                       NULL);
        TWK_GLDEBUG;
    }
    else 
    {
        ...
        glDrawArrays(m_primitiveData.m_primitiveType,
                     0,

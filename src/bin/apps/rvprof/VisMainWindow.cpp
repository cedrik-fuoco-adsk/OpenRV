...
inline void glVertex( const TwkMath::Vec3f &v ) 
{ 
    glVertex3fv( ( GLfloat * )( &v ) ); 
}

inline void glVertex( const TwkMath::Vec4f &v ) 
{ 
...
inline TwkMath::Mat44f getMatrix( GLenum matrix ) 
{
    ...
    // format.
    return TwkMath::Mat44f( m[0], m[4], m[8],  m[12],
                            m[1], m[5], m[9],  m[13],
                            m[2], m[6], m[10], m[14],
                            m[3], m[7], m[11], m[15] );
}
    ...
    QMainWindow(parent)
{
    ...
    connect(m_dockUI.showIdealFramesButton, SIGNAL(stateChanged(int)),
            this, SLOT(showIdealFrames(int)));
    connect(m_dockUI.showEvalTimingButton, SIGNAL(stateChanged(int)),
            this, SLOT(showEvalTiming(int)));
...
bool 
VisMainWindow::eventFilter(QObject *obj, QEvent *event)
{
...
void
VisMainWindow::readFile(const QString& filename)
{
...
#define OFFSET_SINGLE(NAME,FIELD) \
            {#NAME, ((char*)&e.FIELD) - ((char*)&e)} 

            NameOffsetPair offsets[] = {
                OFFSET_PAIR(R,render),
                OFFSET_PAIR(S,swap),
                OFFSET_PAIR(E,eval),
            ...
            for (size_t i = 0; i < parts.size(); i++)
            {
                ...
                if      (nameValue[0] == "GC")   e.gccount = nameValue[1].toInt();
                else if (nameValue[0] == "F")    e.frame = nameValue[1].toInt();
                else if (nameValue[0] == "EST")  e.expectedSyncTime = nameValue[1].toFloat();
                else if (nameValue[0] == "DCO")  e.deviceClockOffset = nameValue[1].toFloat();
                else if (nameValue[0] == "PRUP") e.prefetchUploadPlane = nameValue[1].toFloat();
                else if (nameValue[0] == "RRUP") e.renderUploadPlane = nameValue[1].toFloat();
                else if (nameValue[0] == "RFW")  e.renderFenceWait = nameValue[1].toFloat();
            }
...
void
VisMainWindow::showFile()
{
    if (m_fileViewDialog->isVisible()) m_fileViewDialog->hide();
    else m_fileViewDialog->show();
}
...
VisMainWindow::rangeChanged()
{
    ...
    if (i0 < data.size() && i1 < data.size())
    {
        ...
        if (minRR > 0) m_dockUI.maxVariationLabel->setText(QString("%1 Hz").arg(1.0/ minRR));
        if (maxRR > 0) m_dockUI.minVariationLabel->setText(QString("%1 Hz").arg(1.0/ maxRR));
    }
}
...
GLView::GLView(QWidget* parent, QTextEdit* readout, VisMainWindow* visWin)
    : QOpenGLWidget(parent, Qt::Widget),
      ...
      m_visWin(visWin)
{
    // Create and set the QSurfaceFormat
    QSurfaceFormat format;
    QSurfaceFormat::setDefaultFormat(format);
    setFormat(format); // Apply the format to the QOpenGLWidget
...
void 
GLView::setRange(int start, int end)
{
...
void 
GLView::setTimeRange(float start, float end)
{
    bool foundStart = false;
    bool foundEnd = false;
...
void
GLView::setData(const DataVector& data)
{
    m_data = data;
    ...
    m_matrix = scaleMatrix<float>(Vec(d, d, 1)) 
                * translationMatrix<float>(Vec(-m_startTime + 0.02, y * 0.25, 0));
    */
    float d =  1.0;
...
void
GLView::rebuildIdealFrames()
{
    if (m_actualFPS < 0) return;
}
...
#define APPCOLSTART(r,g,b) m_readoutHtml.append(QString("%1<font color=\"#%2%3%4\">").arg(indent).arg(int(r*255),2,16,QChar('0')).arg(int(g*255),2,16,QChar('0')).arg(int(b*255),2,16,QChar('0')))
#define APPCOLEND()        m_readoutHtml.append(QString("</font>"))
#define APPCOLTXT(r,g,b,s) m_readoutHtml.append(QString("%1<font color=\"#%2%3%4\">&diams;&diams;&diams;&diams; </font><b>%5</b>").arg(indent).arg(int(r*255),2,16,QChar('0')).arg(int(g*255),2,16,QChar('0')).arg(int(b*255),2,16,QChar('0')).arg(s))
...
GLView::generateHtml()
{
    ...
    if (m_readoutSample < 0 || m_readoutSample >= m_data.size()) return;
    ...
    APPITEM ("from begin of GLView::paint, until we wait for swap", e.render);
        ...
        APPITEM ("Internal render time, total time in CompositeRenderer::render", e.internalRender);
            ...
            APPITEMTEST ("userRender", "userRender code (handling \"render\" event).  only time delta is valid.", e.userRender, .6, .6, 1);
            APPITEMTEST ("frameChange", "internal frameChangedMessage processing plug \"frame-changed\" user event.", e.frameChange, 1, 1, .6);

            APPDELTAITEMTEST ("renderFenceWait", "total fence wait time.  should only be non-zero in presentation mode.", e.renderFenceWait, .6, .2, .2);
            APPDELTAITEMTEST ("renderUploadPlane", "total time in uploadPlane().  should only be non-zero if upload did not complete in prefetch.", e.renderUploadPlane, .6, .2, .6);
    ...
    {
        swapText = QString("rendering complete, waiting for buffer swap, <b>equiv rate = %1</b>").arg(swapRate);
    }
    else
    {
    ...
    APPITEM ("time spent in Session::postRender, including eval time if prefetching.", e.prefetch);
        ...
        APPITEM ("total prefetch rendertime, CompositeRenderer::prefetch.", e.prefetchRender);
            ...
            indent = spaces+spaces;
            APPDELTAITEMTEST ("prefetchUploadPlanes", "time spent in uploadPlanes, may be zero of upload is async", e.prefetchUploadPlane, .6, .2, .6);
    ...
    APPBR();
    APP0 ("<b>Cache Node Evaluation Items</b>  (CacheIPNode::evaluate) These may be garbled by timing issues if caching is happening during playback.<br>");
    ...
    APPBR();
    APP3 ("Expected time of next vsync: %1, which is in the %2 (%3 sec from end of render).", 
...
int
GLView::sampleFromMousePosition(float x, float y)
{
...
GLView::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (event->buttons() & Qt::LeftButton)
    {
        Vec t = Vec(dp.x() / w * (m_endTime - m_startTime), 
                    -dp.y() / h * (m_endTime - m_startTime) * (h / w),
                    0);
    ...
    if (event->buttons() & Qt::LeftButton)
    {
        if (event->modifiers() & Qt::ShiftModifier)
        {
            ...
            while (m_rangeStartIndex > 1)
            {
                ...
                if (one.expectedSyncTime == 0.0 || two.expectedSyncTime == 0.0) break;
                if (one.frame > two.frame) break;
                --m_rangeStartIndex;
            }
            ++m_rangeStartIndex;
            ...
            while (frame == m_data[m_rangeEndIndex].frame) --m_rangeEndIndex;
...
void 
GLView::mouseReleaseEvent(QMouseEvent* event)
{
}

void 
GLView::wheelEvent(QWheelEvent* event)
{
    float w = width();
    float h = height();
...
GLView::paintGL()
{
    ...
    if (m_data.empty()) { glFinish(); return; }
	...
	//
	//  Green box, drawn above render box.  corresponds to Session's evaluateForDisplay().
	//
	...
	drawBox (.3, 1, .3, 1, e.eval0, y0, e.eval1, y1);
        ...
        //
        //  Light red.  Internal render time (total time in CompositeRenderer::render()
        //  Disjoint from eval time in total paint time, so draw in 2nd vertical third.
        //
        ...
        //
        //   Next four are subsets of Internal Render, so draw above in top vertical third.
        //
        ...
        //  only be non-zero if texture uploads did not complete during prefetch.
        //
    ...
    for (size_t i = 0; i < m_data.size(); i++)
    {
        ...
        if (false && m_showIdealFrames && i >= m_rangeStartIndex && i <= m_rangeEndIndex)
        {
            ...
            else
            {
                ...
                int previous = m_actualFPS * t0 + m_data[m_rangeStartIndex].frame;

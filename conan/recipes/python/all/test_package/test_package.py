...
def find_python_packages():
    ...
    try:
        ...
        for line in lines:
            ...
            elif len(line_stripped) == 40 and line_stripped.isalnum() and current_version:
...
def test_package_ssl_and_otio(package_info):
    ...
    try:
        ...
        if result.returncode != 0:
            print("  ERROR: Could not get package path")
            return False
...
"""
        ...
        result = subprocess.run([python_exe, "-c", test_script], capture_output=True, text=True, timeout=30)
        ...
        if ssl_ok:
            ...
            print("  FAIL: SSL: Failed")

        if otio_ok:
            print(f"  PASS: OpenTimelineIO: {otio_version}")
        else:
            print("  FAIL: OpenTimelineIO: Failed")
...
def test_built_packages():
    ...
    for pkg in packages:
        print(f"  • Python {pkg['version']} (VFX {pkg['vfx_platform']}) - {pkg['package_id'][:8]}...")
